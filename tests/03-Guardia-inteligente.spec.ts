...
test('guardia inteligentes - flujo completo', async ({ page }) => {
  ...
  await ingresarButton.waitFor({ state: 'visible', timeout: 90000 })
  ...
  const continuarButton2 = page.getByRole('button', { name: 'CONTINUAR' })
  await continuarButton2.waitFor({ state: 'visible', timeout: 90000 })
  await continuarButton2.click()
