...
test('turnos medicos - flujo basico', async ({ authenticatedPage }) => {
  ...
  // Click la primer fecha disponible con texto
  ...
  // Verificar inputs 'N° Área' y 'N° Celular'
...
// ==========================================
//  2do flujo: selección por PROFESIONAL


// Volver a la home de turnos
await page.goto('https://portal-test.galeno.com.ar/socio/turno')
await page.waitForLoadState('domcontentloaded')

// Click en "Nuevo Turno"
const nuevoTurnoButton2 = page.locator('[data-testid="AddIcon"]')
await nuevoTurnoButton2.waitFor({ state: 'visible', timeout: 60000 })
await nuevoTurnoButton2.click()

// Esperar navegación a crear turno
await page.waitForURL('**/socio/turno/crear', { timeout: 90000 })
await expect(page).toHaveURL(/\/socio\/turno\/crear/)
...
// Screenshot
await page.screenshot({ path: 'crear-turno-segunda-vuelta.png', fullPage: true })

//  Seleccionar checkbox "Profesional"


// 1. Intento ideal (por accesibilidad)
let checkboxProfesional = page.getByRole('checkbox', { name: /profesional/i })


if ((await checkboxProfesional.count()) === 0) {
  checkboxProfesional = page.locator('input[type="checkbox"].PrivateSwitchBase-input').nth(1)
}

// Esperar
await checkboxProfesional.waitFor({ state: 'attached', timeout: 60000 })

// Click solo si no está marcado
const isChecked = await checkboxProfesional.isChecked().catch(() => false)

if (!isChecked) {
  await checkboxProfesional.click({ force: true })
}

// Validación
try {
  await expect(checkboxProfesional).toBeChecked()
} catch {
  
  await page.screenshot({ path: 'checkbox-profesional-error.png', fullPage: true })
  throw new Error('No se pudo seleccionar el checkbox Profesional')
}
//  Buscar profesional "gomez"


// Input de profesional 
const profesionalInput = page.locator('#debug')

// Esperar que esté visible
await profesionalInput.waitFor({ state: 'visible', timeout: 60000 })

// Escribir "gomez"
await profesionalInput.fill('gomez')

// Presionar ENTER para disparar búsqueda
await page.keyboard.press('Enter')

//  Seleccionar primer resultado


// Opción esperada: "Gomez Oscar Nereo"
const profesionalOption = page.getByRole('option', { name: /Gomez Oscar Nereo/i })

try {
  await profesionalOption.waitFor({ state: 'visible', timeout: 15000 })
  await profesionalOption.first().click()
} catch (e) {
  // fallback: agarrar la primera opción disponible
...
// Screenshot 
await page.screenshot({ path: 'profesional-seleccionado.png', fullPage: true })


//  Esperar 5 segundos para que se autocompleten los campos
await page.waitForTimeout(5000)

//  Click en botón CONTINUAR
const continuarBtn = page.locator('#btnContinuar')
await continuarBtn.click()
// ==========================================
// calendario (2do flujo)


const calendarXpath2 = '//*[@id="vertical-tabpanel-0"]/div/div[1]/div/div[1]/div/div[2]'
const calendar2 = page.locator(`xpath=${calendarXpath2}`)
await calendar2.waitFor({ state: 'visible', timeout: 90000 })

// Fechas disponibles
const availableDates2 = calendar2.locator('button:not([disabled])')
const datesCount2 = await availableDates2.count()

if (datesCount2 === 0) {
  await page.screenshot({ path: 'calendar2-no-dates.png', fullPage: true })
  throw new Error('No hay fechas disponibles (flujo 2)')
}

// Seleccionar fecha
let dateClicked2 = false

for (let i = 0; i < datesCount2; i++) {
  const d2 = availableDates2.nth(i)
  const txt2 = (await d2.innerText()).replace(/\s+/g, ' ').trim()
...
if (!dateClicked2) {
  await page.screenshot({ path: 'calendar2-date-failed.png', fullPage: true })
  throw new Error('No se pudo seleccionar fecha (flujo 2)')
}

//  Horarios


const timeOptionsXpath2 =
  '//*[@id="vertical-tabpanel-0"]//button[contains(normalize-space(.),":")]'

const timeOptions2 = page.locator(`xpath=${timeOptionsXpath2}`)

await timeOptions2.first().waitFor({ state: 'visible', timeout: 60000 })

const timeCount2 = await timeOptions2.count()

if (timeCount2 === 0) {
  await page.screenshot({ path: 'calendar2-no-times.png', fullPage: true })
  throw new Error('No hay horarios (flujo 2)')
}

// Seleccionar horario
let timeClicked2 = false

for (let i = 0; i < timeCount2; i++) {
  const t2 = timeOptions2.nth(i)
  const txtTime2 = (await t2.innerText()).trim()

  if (!txtTime2) continue
...
if (!timeClicked2) {
  await page.screenshot({ path: 'calendar2-time-failed.png', fullPage: true })
  throw new Error('No se pudo seleccionar horario (flujo 2)')
}

await page.screenshot({
  path: 'flujo2-fecha-hora.png',
  fullPage: true
})

//  Formulario (variables nuevas)


await page.waitForSelector(
  'text=Necesitamos tus datos para comunicarnos con vos',
  { timeout: 90000 }
)

const nAreaInput2 = page.locator('xpath=//*[@id="component-outlinedN° Área"]')
const celularInput2 = page.locator('xpath=//*[@id="component-outlinedN° Celular"]')
const emailInput2 = page.locator('xpath=//*[@id="component-outlinedE-mail"]')

await nAreaInput2.waitFor({ state: 'visible' })
await celularInput2.waitFor({ state: 'visible' })
await emailInput2.waitFor({ state: 'visible' })

await nAreaInput2.fill('011')
await celularInput2.fill('59263547')
await emailInput2.fill('prueba2@gmail.com')

// BOTON CONTINUAR


const continuarFinal2 = page.locator('#btnContinuar')

await continuarFinal2.waitFor({ state: 'visible', timeout: 15000 })
await continuarFinal2.click()

await page.waitForLoadState('networkidle')

await page.screenshot({ path: 'flujo2-after-continuar.png', fullPage: true })
...
const turnoXpath2 =
  '//*[@id="scrollable-auto-tabpanel-0"]/div/div[1]/div/div/div[4]/div'

const turnoItem2 = page.locator(`xpath=${turnoXpath2}`)

await turnoItem2.waitFor({ state: 'visible', timeout: 60000 })
await turnoItem2.click()

await page.waitForURL('**/socio/turno/detalle/0')
await expect(page).toHaveURL(/\/socio\/turno\/detalle\/0/)

await page.screenshot({
  path: 'flujo2-detalle-turno.png',
  fullPage: true
})

// 31. Cancelar turno (flujo 2)

const btnCancelar2 = page.locator('#btnCancelarTurno')

await btnCancelar2.waitFor({ state: 'visible', timeout: 15000 })
await btnCancelar2.click()

let clickedCancelConfirm2 = false

try {
  const siBtn2 = page.getByRole('button', { name: 'SI' })
  await siBtn2.first().click()
  clickedCancelConfirm2 = true
} catch {
  try {
    const siBtnXpath2 = "//button[.//span[normalize-space(text())='SI']]"
    const siBtnXpathLoc2 = page.locator(`xpath=${siBtnXpath2}`)
    await siBtnXpathLoc2.first().click()
    clickedCancelConfirm2 = true
  } catch {
    try {
      const confirmSpan2 = page.locator(
        'xpath=/html/body/div[4]/div[3]/div/div/button[2]/span[1]'
      )
      await confirmSpan2.click()
      clickedCancelConfirm2 = true
    } catch {
      try {
        const confirmBtn2 = page.locator(
          'xpath=/html/body/div[4]/div[3]/div/div/button[2]'
        )
        await confirmBtn2.click()
        clickedCancelConfirm2 = true
      } catch {
        await page.screenshot({
          path: 'flujo2-error-cancel.png',
          fullPage: true
        })
        throw new Error('Flujo 2: no se pudo confirmar cancelación')
      }
    }
  }
}
...
await page.screenshot({
  path: 'flujo2-cancelado.png',
  fullPage: true
})



//  Validar vuelta a home (flujo 2)

await page.waitForURL('https://portal-test.galeno.com.ar/socio/turno', {
  timeout: 90000
})

await expect(page).toHaveURL(
  'https://portal-test.galeno.com.ar/socio/turno'
)
...
try {
  ...
  console.log(' No se pudo tomar screenshot (page cerrada)')
}
